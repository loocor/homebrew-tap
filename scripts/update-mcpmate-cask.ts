...
type RequiredAssets = Record<RequiredAssetKey, Asset>;
...
type ManifestSource =
	...
	| { type: "url"; value: string; tag: string };

const requiredAssets: Record<
	RequiredAssetKey,
	Pick<Asset, "platform" | "arch" | "format">
> = {
...
const releaseTargets: Record<
	ReleaseChannel,
	Pick<ReleaseTarget, "caskToken" | "caskPath">
> = {
...
function parseSource(arguments_: string[]): ManifestSource {
	if (arguments_.length !== 2) {
		fail(
			"Provide exactly one source: --manifest-file <path> or --manifest-url <https-url>",
		);
	}

	const [flag, value] = arguments_;
	if (
		(flag !== "--manifest-file" && flag !== "--manifest-url") ||
		!value ||
		value.startsWith("--")
	) {
		fail(
	...
	if (flag === "--manifest-url") {
		...
		const tag = url.pathname.startsWith(manifestPathPrefix)
			? url.pathname.slice(manifestPathPrefix.length)
			: "";
		if (
			value !== url.toString() ||
			...
			fail(
				"Manifest URL must be canonical: https://public.mcp.umate.ai/downloads/releases/<tag>",
			);
		}
...
function parseManifest(source: string): ReleaseManifest {
	...
	const tagChannel =
		typeof manifest.tag === "string" ? parseReleaseTag(manifest.tag) : null;
	if (!tagChannel) {
	...
	const manifestAssets = manifest.assets as Partial<RequiredAssets>;
	const normalizedAssets: Partial<RequiredAssets> = {};
	for (const [key, expected] of Object.entries(requiredAssets) as [
		RequiredAssetKey,
		(typeof requiredAssets)[RequiredAssetKey],
	][]) {
		...
		if (
			...
			typeof asset.homebrewUrl !== "string" ||
			typeof asset.sha256 !== "string"
		) {
			fail(`Manifest asset is incomplete: ${key}`);
		}
		const expectedExtension =
			expected.format === "appimage" ? ".AppImage" : ".dmg";
		if (
			!safeBasename.test(asset.name) ||
		...
		if (!sha256.test(asset.sha256)) {
			fail(
				`Manifest sha256 must be 64 lowercase hexadecimal characters: ${key}`,
			);
		}
	...
	return {
		...manifest,
		assets: normalizedAssets as RequiredAssets,
	} as ReleaseManifest;
}

function renderAsset(asset: Asset, version: string): string {
	const versionedUrl = asset.homebrewUrl.replace(
		`/v${version}/`,
		"/v#{version}/",
	);
...
function renderLivecheck(releaseChannel: ReleaseChannel): string {
	const versionPattern = String.raw`(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)`;
	const channelPattern =
		releaseChannel === "beta"
			? String.raw`${versionPattern}-beta(?:\.(?:0|[1-9]\d*))?`
			: versionPattern;

	return `  livecheck do
    url "https://github.com/loocor/mcpmate"
    regex(/^v(${channelPattern})$/)
...
function renderCask(manifest: ReleaseManifest, target: ReleaseTarget): string {
	const macosArm = renderAsset(
		manifest.assets["macos-arm64-dmg"],
		manifest.version,
	);
	const macosX64 = renderAsset(
		manifest.assets["macos-x64-dmg"],
		manifest.version,
	);
	const linuxArm = renderAsset(
		manifest.assets["linux-arm64-appimage"],
		manifest.version,
	);
	const linuxX64 = renderAsset(
		manifest.assets["linux-x64-appimage"],
		manifest.version,
	);
	const linuxArmAppImage = renderAppImage(
		manifest.assets["linux-arm64-appimage"],
	);
	const linuxX64AppImage = renderAppImage(
		manifest.assets["linux-x64-appimage"],
	);
	const livecheck = renderLivecheck(target.releaseChannel);
...
async function fetchManifest(
	...
	try {
		...
		if (
			...
			error.message.startsWith("Unable to fetch manifest:")
		) {
			throw error;
		}
...
export async function updateCask(
	...
	const manifestText =
		source.type === "file"
			...
			: await fetchManifest(source.value, fetcher);
