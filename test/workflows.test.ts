...
describe("tap CI workflow", () => {
  test("parses and validates every tracked MCPMate Cask on pull requests and main pushes", async () => {
    ...
    expect(workflow).toContain('brew audit --cask --strict "loocor/tap/$cask_token"');
  ...
  test("runs both supported Linux Homebrew versions and constructs the AppImage artifact", async () => {
    ...
    expect(workflow).toContain("git ls-files 'Casks/mcpmate*.rb'");
    expect(workflow).toContain('HOMEBREW_NO_INSTALL_FROM_API=1 brew install --cask "loocor/tap/$cask_token"');
    ...
    expect(workflow).toContain('brew uninstall --cask "loocor/tap/$cask_token"');
...
describe("maintainer documentation", () => {
  test("documents stable and beta targets without advertising an unavailable stable Cask", async () => {
    ...
    expect(readme).toContain("| Beta | `mcpmate@beta` | `Casks/mcpmate@beta.rb` |");
    expect(readme).toContain("A stable Cask is not published until the first validated stable manifest generates it.");
    expect(readme).not.toContain("brew install --cask loocor/tap/mcpmate\n");
    expect(readme).not.toMatch(/supported.*(?:nightly|debug)|(?:nightly|debug).*supported/i);
  });

  test("documents the deployed Admin contract as the release authority", async () => {
    const readme = await readFile(readmePath, "utf8");

    expect(readme).toMatch(/The deployed Admin exact-tag manifest and tracked download routes are the\s+release authority/);
...
describe("MCPMate update workflow", () => {
  ...
  test("derives the selected Cask only from the validated exact-tag manifest", async () => {
    ...
    expect(workflow).toContain('bun scripts/release-contract.ts "$RELEASE_TAG" "$MANIFEST_URL"');
    expect(workflow).toContain('TARGET_JSON="$(bun scripts/update-mcpmate-cask.ts --manifest-url "$MANIFEST_URL")"');
    expect(workflow).toContain('RELEASE_CHANNEL="$(jq -er \'.releaseChannel\' <<<"$TARGET_JSON")"');
    expect(workflow).toContain('CASK_TOKEN="$(jq -er \'.caskToken\' <<<"$TARGET_JSON")"');
    expect(workflow).toContain('CASK_PATH="$(jq -er \'.caskPath\' <<<"$TARGET_JSON")"');
    expect(workflow).toContain('test "$(jq -er \'.tag\' <<<"$TARGET_JSON")" = "$RELEASE_TAG"');
    expect(workflow).toContain("bun test");
    expect(workflow).toContain('brew tap loocor/tap "$GITHUB_WORKSPACE"');
  ...
  test("uses channel-specific publishing metadata and detects a first stable Cask", async () => {
    ...
    expect(workflow).toContain('git status --porcelain --untracked-files=all -- "$CASK_PATH"');
    expect(workflow).toContain("echo \"changed=false\" >> \"$GITHUB_OUTPUT\"");
    expect(workflow).toContain("if: steps.cask.outputs.changed == 'true'");
    expect(workflow).toContain('BRANCH="chore/mcpmate-${RELEASE_CHANNEL}-${VERSION}"');
    expect(workflow).toContain('TITLE="chore(mcpmate): update ${RELEASE_CHANNEL} to ${VERSION}"');
