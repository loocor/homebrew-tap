...
beforeAll(async () => {
  ...
  originalStableCask = await Bun.file(stableCaskPath).exists() ? await readFile(stableCaskPath, "utf8") : null;
});
afterEach(async () => {
...
async function releaseManifest(
  ...
  return temporaryManifest((manifest) => {
    ...
    for (const asset of Object.values(manifest.assets as Record<string, Record<string, string>>)) {
      asset.name = asset.name.replace("0.3.4", version.split("-")[0].split("+")[0]);
...
describe("update-mcpmate-cask", () => {
  ...
  test("supports manifest URL mode without any alternate source", async () => {
    ...
    expect(requestedUrl).toBe("https://public.mcp.umate.ai/downloads/releases/v0.3.4-beta"); expect(await readFile(betaCaskPath, "utf8")).toContain('version "0.3.4-beta"');
  });
  ...
  test("derives the beta target from a validated beta manifest", async () => {
    ...
    expect(await readFile(betaCaskPath, "utf8")).toContain('cask "mcpmate@beta" do');
  });
  ...
  test("does not create a placeholder stable Cask for a beta manifest", async () => {
    expect((await runUpdater("--manifest-file", fixturePath)).exitCode).toBe(0);
    if (originalStableCask === null) await expect(Bun.file(stableCaskPath).exists()).resolves.toBe(false);
    else expect(await readFile(stableCaskPath, "utf8")).toBe(originalStableCask);
  });
  ...
  test("accepts only beta and beta numeric prerelease tags for the beta channel", async () => {
    for (const version of ["1.2.3-beta", "1.2.3-beta.7"]) {
      ...
      expect(await readFile(betaCaskPath, "utf8")).toContain(`version "${version}"`);
    }
  });
  ...
  test.each([
    ...
    expect((await runUpdater("--manifest-file", path)).stderr).toContain("Manifest releaseChannel does not match the release tag");
  });

  test.each(["alpha", "rc.1", "dev", "debug", "nightly", "preview.1", "beta.preview"])(
    "rejects an unsupported prerelease suffix: %s",
    async (suffix) => {
      const path = await releaseManifest(`1.2.3-${suffix}`, "beta");
      expect((await runUpdater("--manifest-file", path)).stderr).toContain("Manifest tag must be a supported stable or beta release tag");
    },
  );
  ...
  test.each(["01.2.3", "1.02.3", "1.2.03", "1.2.3-beta.01"])(
    ...
    async (version) => {
      const path = await releaseManifest(version, version.includes("beta") ? "beta" : "stable");
      expect((await runUpdater("--manifest-file", path)).stderr).toContain("Manifest tag must be a supported stable or beta release tag");
    },
  );
  ...
  test.each([
    ["literal plus", false],
    ["encoded plus", true],
  ])("rejects SemVer build metadata in tag and URLs: %s", async (_case, encodedTag) => {
    const path = await releaseManifest("1.2.3-beta+build.1", "beta", { encodedTag });
    expect((await runUpdater("--manifest-file", path)).stderr).toContain("Manifest tag must be a supported stable or beta release tag");
  });
  ...
  test("rejects a releaseChannel outside the supported manifest contract", async () => {
    ...
    expect((await runUpdater("--manifest-file", path)).stderr).toContain('Manifest releaseChannel must be "stable" or "beta"');
  });
  ...
  ])("is byte-identical when run twice for the %s channel", async (_case, version, channel, caskPath) => {
    const path = channel === "beta" ? fixturePath : await releaseManifest(version, channel);
    expect((await runUpdater("--manifest-file", path)).exitCode).toBe(0);
  ...
  test("declares symmetric conflicts because stable and beta install the same artifacts", async () => {
    ...
    expect((await runUpdater("--manifest-file", fixturePath)).exitCode).toBe(0);
    expect((await runUpdater("--manifest-file", stableManifest)).exitCode).toBe(0);
    expect(await readFile(betaCaskPath, "utf8")).toContain('conflicts_with cask: "mcpmate"');
    expect(await readFile(stableCaskPath, "utf8")).toContain('conflicts_with cask: "mcpmate@beta"');
  });
  ...
  test("keeps forbidden fallback and lifecycle artifacts out of every supported channel", async () => {
    ...
    for (const path of [betaCaskPath, stableCaskPath]) {
      ...
      expect(cask).not.toContain("github.com/loocor/mcpmate");
      ...
      expect(cask).toContain("does not terminate services or remove ~/.mcpmate");
    }
  });

  test("restores the tracked beta Cask after integration tests", async () => { expect(await readFile(betaCaskPath, "utf8")).toBe(originalCask); });
});
